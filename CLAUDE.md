...
│   ├── chapter02.md
│   └── chapter03.md
├── static/images/         # PERMANENT: Hugo-served images
│   ├── chapter01/         # Static images for Chapter 1
│   ├── chapter02/         # Static images for Chapter 2
│   └── chapterXX/         # Create for each chapter
├── codes/                 # Executable Python examples
│   ├── chapter01/         # Chapter-specific code files
│   │   └── images/        # GENERATED: Code output images
│   ├── chapter02/
│   │   └── images/
...
├── TABLE_OF_CONTENTS.md   # Complete book structure
...
3. Structure: Introduction → Concepts → Mathematics → Applications
...
4. Add "핵심 인사이트" (Key Insights) section
...
**Rules**:
- ✅ **ALWAYS use `/images/` paths in markdown files** (NOT `../../codes/` paths)
- ✅ Copy representative images from `codes/chapterXX/images/` to `static/images/chapterXX/`
- ✅ These images persist and are version-controlled
- ✅ Hugo serves them from `/images/` URL path
- ✅ Update static images when code execution produces new/improved versions
...
**Problem**: matplotlib defaults to fonts that don't support Korean characters (한글), causing text to display as boxes (□□□).
...
- ✅ Every script that imports `matplotlib.pyplot`
- ✅ Add immediately after imports, before any plotting code
- ✅ Include even if existing code has partial font settings
- ✅ Both new scripts and when editing existing scripts
...
❌ **Wrong**: `plt.rcParams['font.family'] = 'DejaVu Sans'` (Korean won't render)
❌ **Wrong**: No font configuration (relies on system defaults)
❌ **Wrong**: Only setting font in one script per chapter
✅ **Correct**: Font configuration in EVERY matplotlib script
✅ **Correct**: Using the standard font priority chain above
...
**차트 해석**:
- **상단 패널**: [Describe what's shown and what it means]
- **하단 패널**: [Describe what's shown and what it means]
...
- [ ] All Korean text displays properly (no □□□)
...
- ✅ Execution results included in markdown
- ✅ Charts with interpretations
- ✅ Key insights summarized
- ✅ Korean fonts configured properly

### Level 3: Comprehensive (Premium)
- ✅ All Level 2 requirements
- ✅ Multiple examples with varying parameters
...
### ❌ Pitfall 2: Missing Font Configuration
**Problem**: Korean text displays as □□□ in charts
...
### ❌ Pitfall 3: Incomplete Chart Explanations
...
### ❌ Pitfall 4: Theory Without Execution
...
### ❌ Pitfall 6: Inconsistent Image Quality
...
### ❌ Pitfall 7: Forgetting to Copy Images
...
### ❌ Pitfall 8: No Key Insights Summary
...
### ✅ Success Pattern: Complete Enhancement Flow

1. **Code** → Write with Korean font config
2. **Execute** → Run and verify output
3. **Copy** → Move images to static/
4. **Document** → Add results + interpretations
5. **Test** → Preview in Hugo
6. **Validate** → Check all items in quality checklist
...
**Part 1 (Chapters 1-4): Foundations & Data** ✅ **COMPLETE - Level 2 Enhanced**

All chapters include:
- ✅ Complete Korean theory with LaTeX formulas
- ✅ Executable Python code with Korean font configuration
- ✅ Actual execution results embedded in markdown
- ✅ Detailed chart interpretations in Korean
- ✅ Key insights and practical applications
- ✅ All images in `static/images/` with `/images/` references
...
**Part 5 (Chapters 14-16): Advanced Techniques** 📋 PLANNED
**Part 6 (Chapters 17-18): Real-World Application** 📋 PLANNED
