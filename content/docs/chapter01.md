...
## 1.5 첫 번째 Python 스크립트: Hello Trading
...
차트는 세 부분으로 구성됩니다:
1. **상단**: 주가 추이 - 시간에 따른 가격 변화
2. **중단**: 일일 수익률 - 각 날짜별 수익률 (%)
3. **하단**: 누적 수익률 - 초기 투자금 대비 총 수익
...
백테스팅은 강력한 도구지만 만능은 아닙니다. 과거의 성과가 미래를 보장하지 않습니다. 백테스팅은 전략을 검증하는 **하나의 도구**일 뿐이며, 실전 트레이딩에서는 시장 상황, 리스크 관리, 심리적 요인 등을 종합적으로 고려해야 합니다.
